...
//	-------------------------------------------------------------
	...
	var buttonDownToPageHeightRatio = 1/10;	//for mouse-down
...
//	images :
	...
	var playButtonDown = new Image();
	var instructionsButtonDown = new Image();
	var settingsButtonDown = new Image();
	var creditsButtonDown = new Image();
	...
	//down can use the same handling of over
	playButtonDown.src = "images/GameMainMenu/Play_Down.png";
	instructionsButtonDown.src = "images/GameMainMenu/Instructions_Down.png";
	settingsButtonDown.src = "images/GameMainMenu/Settings_Down.png";
	creditsButtonDown.src = "images/GameMainMenu/Credits_Down.png";
...
//	buttons and misc data :
	...
	//must be same order of normal button push!
	buttonsDown.push(playButtonDown);
	buttonsDown.push(instructionsButtonDown);
	buttonsDown.push(settingsButtonDown);
	buttonsDown.push(creditsButtonDown);

	setButtonHeightAndYPosition(buttonsOver , buttonDownToPageHeightRatio);
...
//	onload functions : 
	...
	playButtonDown.onload = function(){setButtonStats(playButtonDown , buttonsDown);}

	instructionsButtonDown.onload = function(){setButtonStats(instructionsButtonDown , buttonsDown);}

	settingsButtonDown.onload = function(){setButtonStats(settingsButtonDown , buttonsDown);}

	creditsButtonDown.onload = function(){setButtonStats(creditsButtonDown , buttonsDown);}
...
//	other private functions :
	...
	function setButtonStats(menuButton , array){
		...
		array[array.indexOf(menuButton)].isDown = false;
	...
	function resetMouseDown(array){
		for(var i = 0 ; i < array.length ; i++)
			array[i].isDown = false;
	}
...
//	event listener implementations :
	...
	function checkPress(){
		if (playButtonOver.isOver){
			buttonsDown[buttonsOver.indexOf(playButtonOver)].isDown = true;
		}

		if(instructionsButtonOver.isOver){
			buttonsDown[buttonsOver.indexOf(instructionsButtonOver)].isDown = true;
		}

		if(settingsButtonOver.isOver){
			buttonsDown[buttonsOver.indexOf(settingsButtonOver)].isDown = true;
		}

		if(creditsButtonOver.isOver){
			buttonsDown[buttonsOver.indexOf(creditsButtonOver)].isDown = true;
		}
	}
	...
	function eventListenerRemover(){
		this.removeEventListener("mousedown", checkPress);
...
//	event listeners :
	...
	canvas.addEventListener("mousedown", checkPress);
