...
//	-------------------------------------------------------------
	...
	moveNodesButton.src = "images/createGame/Move_Nodes_Button.png";
	setEdgePropertiesButton.src = "images/createGame/Set_Edge_Properties_Button.png";
	...
	moveNodesButton_Over.src = "images/createGame/Move_Nodes_Button_Over.png";
	setEdgePropertiesButton_Over.src = "images/createGame/Set_Edge_Properties_Button_Over.png";
	...
	moveNodesButton_Active.src = "images/createGame/Move_Nodes_Button_Active.png";
	setEdgePropertiesButton_Active.src = "images/createGame/Set_Edge_Properties_Button_Active.png";
...
//////////////////////////getting, setting, creating and destroying functions
	...
	function recolorEdgeBetween(nodeIndex1 , nodeIndex2 , newColor){
		recolorEdgeSingleDirection(nodeIndex1 , nodeIndex2 , newColor);
		recolorEdgeSingleDirection(nodeIndex2 , nodeIndex1 , newColor);
	}

	function recolorEdgeSingleDirection(nodeIndexFrom , nodeIndexTo , newColor){
		for(var j = 0 ; j < nodes[nodeIndexFrom].edges.length ; j++){
			if(nodes[nodeIndexFrom].edges[j].pointedNodeID == nodes[nodeIndexTo].id){
				nodes[nodeIndexFrom].edges[j].color = newColor;
			}
		}
	}


	function setWeightEdgeBetween(nodeIndex1 , nodeIndex2 , newWeight){
        setWeightEdgeSingleDirection(nodeIndex1 , nodeIndex2 , newWeight);
        setWeightEdgeSingleDirection(nodeIndex2 , nodeIndex1 , newWeight);
	}

	function setWeightEdgeSingleDirection(nodeIndexFrom , nodeIndexTo , newWeight){
		for(var j = 0 ; j < nodes[nodeIndexFrom].edges.length ; j++){
			if(nodes[nodeIndexFrom].edges[j].pointedNodeID == nodes[nodeIndexTo].id){
				nodes[nodeIndexFrom].edges[j].weight = newWeight;
			}
		}
	}
	...
	function checkClick(){
		...
		if(currentMode == buttons.indexOf(createNodeButton)){	//Create Nodes
			...
			for(var i = 0 ; i < nodes.length ; i++){
				for(var j = 0 ; j < nodes[i].edges.length ; j++){
					...
					if(isMouseOverEdgeArea(i , getNodesIndexFromNodeID(nodes[i].edges[j].pointedNodeID))){
						...
						if(allowConsoleMessages)
							console.log("removed edges of indexes: " + i + " (with edge " + j + ")" + " and " + (getNodesIndexFromNodeID(nodes[i].edges[j].pointedNodeID)));
			...
			for(var i = 0 ; i < nodes.length ; i++){
				for(var j = 0 ; j < nodes[i].edges.length ; j++){
					if(isMouseOverEdgeArea(i , getNodesIndexFromNodeID(nodes[i].edges[j].pointedNodeID))){
						var availableColors = ["cyan" , "red" , "aliceblue" , "antiquewhite" , "aqua" , "aquamarine" , "azure" , "beige" ,  "bisque" ,
						                       "black" , "blanchedalmond" , "blue" , "blueviolet" , "brown" , "burlywood" , "cadetblue" , "chartreuse" , 
						                       "chocolate" , "coral" , "cornflowerblue" , "cornsilk" , "crimson" , "cyan" , "darkblue" , "darkcyan" , 
						                       "darkgoldenred" , "darkgray" , "darkgreen" , "darkkhaki" , "darkmagenta" , "darkolivegreen" , "darkorange" ,
						                       ...
						                       "yellowgreen"]
						var colorStr = "";
						for(var co = 0 ; co < availableColors.length ; co++){
							colorStr += availableColors[co];
							if(co != availableColors.length - 1)
								colorStr += " , "; 
						}
						if(allowConsoleMessages)
							console.log("Editing node " + i + " , edge " + j);
						var colorSelected = prompt("Which color would you like it to be?\n(Leave blank if you don't want to change it , write 'options' if you want a list of available names)" + "\nCurrent color: " + nodes[i].edges[j].color + ")");
						if(colorSelected == "options")
							colorSelected = prompt("The available Color names are:\n" + colorStr);
						if(colorSelected != ""){
							recolorEdgeBetween(getNodesIndexFromNodeID(i) , nodes[i].edges[j].pointedNodeID , colorSelected);
						}

						var weightSelected = prompt("What weight would you like it to be?\n(Leave blank if you don't want to change it\nCurrent weight: " + nodes[i].edges[j].weight + ")");
						if(weightSelected != "")
							setWeightEdgeBetween(getNodesIndexFromNodeID(i) , nodes[i].edges[j].pointedNodeID , weightSelected);
					}
				}
			}
		}
